window.addEventListener("DOMContentLoaded", function () {
    // ======================================================================
    // 🔊 ROBUST AUDIO UNLOCK SYSTEM (Works on Vercel / All Browsers)
    // ======================================================================
    ...
    const unlockAudio = function () {
        ...
        // 1. Unlock Babylon.js audio engine
        ...
        // 2. Also resume any raw AudioContext that Babylon might be using
        try {
    ...
    // Attach unlock to every possible first-gesture event
    ...
    function updateMenuUI() {
        ...
        let isUnlocked =
            gameData.unlockedCars.includes(currentCarIndex) ||
            carConfig[currentCarIndex].price === 0;
    ...
    function switchCar(dir) {
        ...
        loadedCars.forEach((c) => {
            if (c.root) {
                ...
                c.root.getChildMeshes(false).forEach((m) => m.setEnabled(false));
            }
        });
        ...
        if (activeCar && activeCar.root) {
            activeCar.root.setEnabled(true);
            activeCar.root.getChildMeshes(false).forEach((m) => m.setEnabled(true));
        }
        ...
        loadedRoadSets.forEach((set, idx) => {
            ...
            set.forEach((mesh) => {
                ...
                if (mesh.getChildMeshes)
                    mesh.getChildMeshes(false).forEach((m) => m.setEnabled(isVis));
            });
        });
    ...
    bindBtn("exitBtn", "click", () => {
        if (navigator.app) navigator.app.exitApp();
            ...
            window.close();
            alert("App Closed! Press back button to exit.");
        }
    });
    ...
    // ======================================================================
    // 🔊 SAFE SOUND PLAYER HELPERS
    // ======================================================================

    /**
     * Safely play a BABYLON.Sound.
     * Ensures AudioContext is running first, then plays.
     */
    ...
    /**
     * Safely set volume of a BABYLON.Sound.
     */
    function safeVolume(sound, vol) {
        if (!sound) return;
        try {
            sound.setVolume(vol);
        } catch (e) {}
    ...
    // ======================================================================
    // 🎵 SOUND FLOW CONTROLLER
    // Three states: COUNTDOWN → DRIVING → CRASHED
    // ======================================================================

    /** Called once per countdown tick (5,4,3,2,1,GO) */
    function playCountSound() {
        safeStop(sfxCar);   // car must be silent during countdown
        safePlay(sfxCount);
    }

    /** Called when countdown finishes — starts looping car engine */
    function startCarEngine() {
        if (!sfxCar) return;
        try {
            safeVolume(sfxCar, 0.8);
            if (!sfxCar.isPlaying) {
                // Ensure loop is set
                sfxCar.loop = true;
                safePlay(sfxCar);
    ...
    /** Called on coin collection — duck engine, play coin, restore engine */
    function playCoinWithDucking() {
        // Play coin sound
        safePlay(sfxCoin);

        // Duck car engine
        if (sfxCar && sfxCar.isPlaying) {
            safeVolume(sfxCar, 0.1);
            if (carAudioDuckTimer) clearTimeout(carAudioDuckTimer);
            carAudioDuckTimer = setTimeout(() => {
                // Restore engine only if still playing game
                if (gameState === "PLAYING" && sfxCar) {
                    safeVolume(sfxCar, 0.8);
                }
            }, 600);
        }
    ...
    /** Called on crash — stop engine, play explosion */
    function playCrashSound() {
        // Kill any pending duck timer first
        if (carAudioDuckTimer) {
            clearTimeout(carAudioDuckTimer);
            carAudioDuckTimer = null;
        }
        safeStop(sfxCar);
        safePlay(sfxCrash);
    }
    ...
    function startEngineLoad() {
        ...
        try {
            ...
            console.log("3D Engine Error: " + err);
        }
    }
    ...
    const createScene = function () {
        ...
        // ======================================================================
        // 🎵 SOUND SYSTEM — Load all sounds once, play via helpers above
        // ======================================================================

        // count.mp3  → plays on each countdown tick (no loop)
        sfxCount = new BABYLON.Sound("countSfx", "assets/sound/count.mp3", s, null, {
            spatialSound: false,
            autoplay: false,
            loop: false,
            volume: 1.0,
        ...
        sfxCar = new BABYLON.Sound("carSfx", "assets/sound/car.mp3", s, null, {
            loop: true,
            autoplay: false,
            spatialSound: false,
            volume: 0.8,
        ...
        sfxCoin = new BABYLON.Sound("coinSfx", "assets/sound/coin.mp3", s, null, {
            spatialSound: false,
            autoplay: false,
            loop: false,
            volume: 1.0,
        ...
        sfxCrash = new BABYLON.Sound(
            "crashSfx",
            "https://playground.babylonjs.com/sounds/explosion.wav",
            s,
            null,
            {
                spatialSound: false,
    ...
    function buildCars(s) {
        carHitbox = BABYLON.MeshBuilder.CreateBox(
            "carHitbox",
            { width: 1.2, height: 1.0, depth: 2.5 },
            s
        );
        ...
        carConfig.forEach((cfg, idx) => {
            BABYLON.SceneLoader.ImportMesh(
                ...
                (newMeshes) => {
                    ...
                    newMeshes.forEach((m) => {
                        if (!m.parent) m.parent = masterCar;
                    });
                    ...
                    if (cfg.scale === "AUTO") {
                        ...
                        masterCar.scaling = new BABYLON.Vector3(cfg.scale, cfg.scale, cfg.scale);
                    }

                    masterCar.position = new BABYLON.Vector3(0, cfg.yOffset - 0.5, 0);
                    if (cfg.rotY !== 0) {
                        masterCar.rotationQuaternion = null;
                        masterCar.rotation.y = cfg.rotY;
                    }
                ...
                null,
                () => {
                    console.log("CAR MISSING: " + cfg.file);
                    let fallback = BABYLON.MeshBuilder.CreateBox(
                        "fallbackCar",
                        { width: 1.2, height: 1.0, depth: 2.5 },
                        s
                    );
                    let mat = new BABYLON.StandardMaterial("fallMat", s);
                    mat.emissiveColor = new BABYLON.Color3(0, 0.5, 1);
                    fallback.material = mat;
                    fallback.parent = carHitbox;
                    fallback.setEnabled(idx === 0);
            ...
            BABYLON.SceneLoader.ImportMesh(
                ...
                (newMeshes) => {
                    ...
                    newMeshes.forEach((mesh) => {
                        ...
                        if (mesh.material && cfg.emissiveBoost > 0)
                    ...
                    let rScale =
                        cfg.scale === "AUTO" ? 24 / width : cfg.scale || 1;
                    ...
                    masterRoad.position.y = -10000;
                    masterRoad.setEnabled(false);
                    ...
                    for (let i = 0; i < 15; i++) {
                        ...
                        if (clone.getChildMeshes)
                            clone.getChildMeshes(false).forEach((m) => m.setEnabled(isVisible));
                ...
                null,
                () => {
                    console.log("ROAD MISSING: " + cfg.file);
                    let fallbackRoad = BABYLON.MeshBuilder.CreateBox(
                        "fallRoad",
                        { width: 24, height: 1, depth: 50 },
                        s
                    );
                    let mat = new BABYLON.StandardMaterial("fallRoadMat", s);
                    mat.emissiveColor = new BABYLON.Color3(1, 0, 1);
                    fallbackRoad.material = mat;
                    fallbackRoad.position.y = -10000;
                    fallbackRoad.setEnabled(false);
    ...
    function buildPools(s) {
        for (let i = 0; i < 30; i++) {
            let obs = BABYLON.MeshBuilder.CreateBox(
                "obs" + i,
                { width: 2.0, height: 1.5, depth: 2.0 },
                s
            );
            obs.material = matObstacle;
            obs.position.y = -1000;
            obs.isVisible = false;
        ...
        for (let i = 0; i < 15; i++) {
            let coin = BABYLON.MeshBuilder.CreateCylinder(
                "coin" + i,
                { diameter: 1.2, height: 0.3 },
                s
            );
            coin.material = matCoin;
    ...
    function spawnItem() {
        ...
        if (carHitbox) {
            obj.mesh.position.y =
                carHitbox.position.y + (isCoin ? 0.5 : 0.25) + activeRoadOffset;
        }
    }

    // ======================================================================
    // 🎮 CONTROLS (SMOOTH SLIDE, BRAKE, NITRO)
    // ======================================================================
    ...
    // ======================================================================
    // ♾️ MASTER TREADMILL GAME LOOP (SMOOTH FLUID MOTION)
    // ======================================================================
    function setupGameLoop() {
        ...
        scene.onPointerObservable.add((info) => {
            ...
            if (info.type === BABYLON.PointerEventTypes.POINTERUP) {
                let endX =
                    ev.clientX || (ev.changedTouches && ev.changedTouches[0].clientX);
        ...
        scene.onBeforeRenderObservable.add(() => {
            ...
            if (gameState === "MENU" && carHitbox) {
                carHitbox.position.x = 0;
                return;
            }
            if (gameState !== "PLAYING" || isPaused) return;
            ...
            if (carHitbox) {
                carHitbox.position.x = BABYLON.Scalar.Lerp(
                    carHitbox.position.x, targetX, 0.1 * dt
                );
                carHitbox.rotation.z = BABYLON.Scalar.Lerp(
                    carHitbox.rotation.z,
                    (targetX - carHitbox.position.x) * -0.08,
                    0.1 * dt
                );
            }
            ...
            if (camera && carHitbox) {
                ...
                let targetFov = isNitroActive
                    ? 1.2
                    : window.innerWidth / window.innerHeight < 1 ? 1.3 : 0.9;

                camera.position.x = 0;
                camera.position.z = BABYLON.Scalar.Lerp(
                    camera.position.z, targetCamZ, 0.05 * dt
                );
            ...
            // 🌟 COIN COLLISIONS + AUDIO DUCKING 🌟
            for (let c of coins) {
                if (!c.active) continue;
                c.mesh.position.z -= currentSpeed;
                c.mesh.rotation.y += 0.05 * dt;
    ...
    bindBtn("startBtn", "click", () => {
        ...
        // ── Stop everything, clear any ducking timer ──
        if (carAudioDuckTimer) { clearTimeout(carAudioDuckTimer); carAudioDuckTimer = null; }
        safeStop(sfxCar);
        safeStop(sfxCoin);
        safeVolume(sfxCar, 0.8); // Reset volume before game starts

        // ── Countdown 5 → 4 → 3 → 2 → 1 → GO! → PLAYING ──
        gameState = "COUNTDOWN";
        ...
        countdownInterval = setInterval(() => {
            ...
            if (count > 0) {
                ...
                // count === -1  →  hide display, start game + engine
                clearInterval(countdownInterval);
                cdDiv.style.display = "none";
                gameState = "PLAYING";
                startCarEngine(); // 🚗 car.mp3 starts looping here
            }
        }, 1000);
    });
