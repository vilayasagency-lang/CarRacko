window.addEventListener("DOMContentLoaded", function () {
    ...
    // 🌟 FIX 1: LANES REDUCED TO STAY ON SCREEN PERFECTLY 🌟
    const laneX = [-1.6, 0, 1.6]; 
    let obstacles =[], coins =[];
    ...
    const createScene = function () {
        ...
        // 🌟 FIX 2: NATIVE WEBAUDIO (NO MEDIA NOTIFICATIONS!) 🌟
            ...
            BABYLON.SceneLoader.ImportMesh("", "assets/", cfg.file, s, 
                (newMeshes) => {
                    ...
                    if (cfg.scale === "AUTO") {
                        let maxDim = 0.1;
                        newMeshes.forEach(m => {
                            // 🌟 FIX 3: Only check dimensions of visible meshes to prevent tiny sizes
                            if(m.getTotalVertices() > 0) {
                                m.computeWorldMatrix(true);
                                let b = m.getBoundingInfo().boundingBox;
                                let dimX = b.maximumWorld.x - b.minimumWorld.x;
                                let dimY = b.maximumWorld.y - b.minimumWorld.y;
                                let dimZ = b.maximumWorld.z - b.minimumWorld.z;
                                let localMax = Math.max(dimX, dimY, dimZ);
                                if(localMax > maxDim && localMax < 50) maxDim = localMax;
                            }
                        });
                        // Force all cars to be exactly identical in size (2.0 Units)
                        let autoScale = 2.0 / maxDim; 
    ...
    function buildPools(s) {
        for (let i = 0; i < 30; i++) {
            let obs = BABYLON.MeshBuilder.CreateBox("obs" + i, { width: 1.8, height: 1.5, depth: 1.8 }, s);
            obs.material = matObstacle; obs.position.y = -1000; obs.isVisible = false;
            obstacles.push({ mesh: obs, active: false });
        }
        for (let i = 0; i < 15; i++) {
            let coin = BABYLON.MeshBuilder.CreateCylinder("coin" + i, { diameter: 1.0, height: 0.3 }, s);
    ...
    function setupGameLoop() {
        ...
        scene.onBeforeRenderObservable.add(() => {
            ...
            if(camera && carHitbox) {
                ...
                let targetFov = isNitroActive ? 1.2 : ((window.innerWidth / window.innerHeight) < 1 ? 1.4 : 1.0);
    ...
    function adjustCameraForPortrait() {
        if (!camera) return;
        camera.fov = (window.innerWidth / window.innerHeight) < 1 ? 1.4 : 1.0;
    }
    ...
    bindBtn("startBtn", "click", () => {
        ...
        if (BABYLON.Engine.audioEngine) {
            ...
            if (BABYLON.Engine.audioEngine.audioContext && BABYLON.Engine.audioEngine.audioContext.state !== 'running') {
