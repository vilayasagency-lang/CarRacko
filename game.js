window.addEventListener("DOMContentLoaded", function () {
    // ======================================================================
    // 🔊 AUDIO UNLOCKER FOR MOBILE WEBVIEWS (Forces Sound to Play)
    // ======================================================================
    ...
    // ======================================================================
    // 🛠️ SAFE DOM BINDER (Yeh game ko crash hone se bachayega)
    // ======================================================================
    ...
    // Yeh function assets load hone par call hota hai
    ...
    function startEngineLoad() {
        ...
        try {
            ...
            let checkInt = setInterval(() => {
                // 🌟 FIX: Total assets ab Cars + Roads + 4 Sounds ko mila kar check honge!
                let totalAssets = carConfig.length + roadConfig.length + 4; 
                
                if (assetsLoadedCount >= totalAssets) { 
                    clearInterval(loadInt); clearInterval(checkInt);
                    let pb = document.getElementById("progressBar");
    ...
    const createScene = function () {
        ...
        // ======================================================================
        // 🎵 PERFECT SOUND SYSTEM (Wait for load to finish + Guaranteed Playback)
        // ======================================================================
        // checkAssetsLoaded callback add kiya gaya hai taaki game download hone tak ruke.
        sfxCount = new BABYLON.Sound("countSfx", "assets/sound/count.mp3", s, checkAssetsLoaded, { spatialSound: false, volume: 1.0 });
        sfxCar = new BABYLON.Sound("carSfx", "assets/sound/car.mp3", s, checkAssetsLoaded, { loop: true, autoplay: false, spatialSound: false, volume: 0.8 });
        sfxCoin = new BABYLON.Sound("coinSfx", "assets/sound/coin.mp3", s, checkAssetsLoaded, { spatialSound: false, volume: 1.0 });
        
        // Crash sound remote server se
        sfxCrash = new BABYLON.Sound("crashSfx", "https://playground.babylonjs.com/sounds/explosion.wav", s, checkAssetsLoaded, { spatialSound: false, volume: 1.0 });
    ...
    bindBtn("playPauseBtn", "click", () => {
        if (gameState === "PLAYING") {
            ...
            if (isPaused && sfxCar) sfxCar.pause();
        ...
        scene.onBeforeRenderObservable.add(() => {
            ...
            for (let c of coins) {
                ...
                if (c.mesh.isVisible && zDist < 2.5 && xDist < 1.5) {
                    ...
                    if (sfxCoin) { try { sfxCoin.play(); } catch(e){} }
                }
    ...
    bindBtn("startBtn", "click", () => {
        ...
        if (sfxCount) { try { sfxCount.play(); } catch(e){} }
        ...
        countdownInterval = setInterval(() => {
            ...
            if (count > 0) {
                ...
                if (sfxCar) { try { sfxCar.play(); } catch(e){} }
            }
        }, 1000);
    });
    ...
    function gameOver() {
        ...
        if(sfxCar) sfxCar.pause();
        if(sfxCrash) { try { sfxCrash.play(); } catch(e){} }
