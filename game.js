window.addEventListener("DOMContentLoaded", function () {
    // ======================================================================
    // 🔊 AGGRESSIVE AUDIO UNLOCKER (Forces Browser to Play Sound)
    // ======================================================================
    ...
    let carAudioDuckTimer = null; // Timer for dynamic audio ducking (coin collection)
    ...
    function setupGameLoop() {
        ...
        scene.onBeforeRenderObservable.add(() => {
            ...
            // 🌟 COLLISION MATH & AUDIO DUCKING (COINS) 🌟
            for (let c of coins) {
                ...
                if (c.mesh.isVisible && zDist < 2.5 && xDist < 1.5) {
                    ...
                    // 👉 2. DUCK CAR ENGINE SOUND (Professional Effect)
                    if (sfxCar && sfxCar.isPlaying) {
                        try {
                            sfxCar.setVolume(0.1); // Thoda band (volume kam)
                            if (carAudioDuckTimer) clearTimeout(carAudioDuckTimer);
                            carAudioDuckTimer = setTimeout(() => {
                                if (gameState === "PLAYING" && sfxCar) {
                                    sfxCar.setVolume(0.8); // Turant wapas full volume
                                }
                            }, 600); // 600ms ducking window
                        } catch(e){}
                    }
                }
    ...
    bindBtn("startBtn", "click", () => {
        ...
        if (sfxCar) { 
            sfxCar.stop(); 
            sfxCar.setVolume(0.8); // Ensure volume is normal before starting
        }
        if (carAudioDuckTimer) clearTimeout(carAudioDuckTimer);
        ...
        // 👉 Start playing count.mp3 on every tick
        ...
        countdownInterval = setInterval(() => {
            ...
            if (count > 0) {
                ...
                // 👉 100% Guaranteed Car Engine Sound Start
