window.addEventListener("DOMContentLoaded", function () {
    ...
    // Har touch aur click par audio ko zinda (active) rakhega
    ...
    function startEngineLoad() {
        ...
        try {
            ...
            // Bypass wait mechanism for robust load
            ...
            setTimeout(() => {
                clearInterval(checkInt);
                finishLoadingProcess();
            }, 10000); // 10 Second Force load fallback
        ...
        scene.onBeforeRenderObservable.add(() => {
            ...
            for (let c of coins) {
                ...
                if (c.mesh.isVisible && zDist < 2.5 && xDist < 1.5) {
                    ...
                    // 🎵 PLAY FRESH COIN SOUND EVEYTIME 🎵
                    if (sfxCoin && sfxCoin.isReady()) {
                        sfxCoin.stop(); // Stop previous overlap
                        sfxCoin.play();
                    }
                }
    ...
    bindBtn("startBtn", "click", () => {
        ...
        if (sfxCount && sfxCount.isReady()) { sfxCount.stop(); sfxCount.play(); }
        ...
        countdownInterval = setInterval(() => {
            ...
            if (count > 0) {
                ...
                if (sfxCar && sfxCar.isReady()) sfxCar.play();
            }
        }, 1000);
    });
    ...
    function gameOver() {
        ...
        if(sfxCar && sfxCar.isPlaying) sfxCar.pause();
        if(sfxCrash && sfxCrash.isReady()) { sfxCrash.stop(); sfxCrash.play(); }
